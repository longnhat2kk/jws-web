...
(function ($) {
  ...
  jwsThemeWooModule = (function () {
    return {
        ...
        $("body.single-product").on("adding_to_cart", function () {
          var checkExist = setInterval(function () {
            if ($("body > .woocommerce-error").length) {
              ...
              var loc = $("body > .woocommerce-error")
                .appendTo(".jws-notices")
                .wrap('<div class="note-item"></div>');
      ...
      checkout_input: function () {
        ...
        $("#customer_details .input-text").on(
          "change keyup focus",
          function () {
          ...
          $(".country_select").each(function () {
            if ($("option:selected", this).val().length > 0) {
              $("option:selected", this)
                .parents(".form-row")
                .addClass("entering_text");
            } else {
              $("option:selected", this)
                .parents(".form-row")
                .removeClass("entering_text");
            }
          });
        }
        checkselect();
            ...
            if (typeof _this !== "undefined" && $("#" + _this).length) {
              ...
              if (_wrap.length) {
                ...
                if (_wrap.find(".jws-error").length) {
                  ...
                  $("#" + _this).after(
                    '<span class="jws-error">' + _appent + "</span>"
                  );
                }
      ...
      jws_load_select_attr: function (variations_global) {
        ...
        if (
          $(product_item).find(".jws-org-price.hidden").length <= 0 &&
          $(product_item).find(".price-item").length
        ) {
          var _first_price = $(product_item).find(".price-item").eq(0);
          $(_first_price).after(
            '<div class="jws-org-price hidden">' +
              $(_first_price).html() +
              "</div>"
          );
        }
         ...
         */
        if (
          typeof $(variations_global).attr("data-product-id") === "undefined"
        ) {
          $(variations_global).attr(
            "data-product-id",
            $(product_item).find(".add_to_cart_button").data("product_id")
          );
        }
        ...
        if (count_attr !== selected_count) {
          ...
          $(add_to_cat_btn)
            .attr("data-product_id", _product_id)
          ...
          add_to_box
            .removeAttr("data-variation")
            .addClass("block_event")
            .html('<i class="jws-icon-glyph-23"></i>' + _select_text);
        } else {
          for (i in variations) {
            var total_attr = 0,
            ...
            for (var k_select in selected_attr) {
              if (
                attrs[selected_attr[k_select].key] === "" ||
                attrs[selected_attr[k_select].key] ===
                  selected_attr[k_select].value
              ) {
                finded = true;
              } else {
                finded = false;
          ...
          if (variation_end && count_attr == total_attr) {
            ...
            if (
              ...
              typeof old_image_gallery === "undefined"
            ) {
            ...
            add_to_box
              .attr("data-variation", variation_end.variation_id)
              .removeClass("block_event")
              .html(
                '<i class="jws-icon-glyph-23"></i>' + jws_script.add_to_box_text
              );
      ...
      addToCartAllTypes: function () {
        ...
        $("body").on(
          "submit",
          ".main-product form.cart:not(.external_form)",
          function (e) {
            e.preventDefault();
            var $form = $(this),
      ...
      shop_sidebar: function () {
        ...
        $(document).on("click", function (e) {
          if (
            $(e.target).is(
              ".shop-sidebar,.shop-sidebar *,.toggle-shop-sidebar,.toggle-shop-sidebar *"
            ) === false
          ) {
      ...
      related_slider: function () {
        ...
        $(".related-slider").not(".slick-initialized").slick({
          swipeToSlide: true,
          prevArrow:
            '<span class="jws-carousel-btn prev-item"><i aria-hidden="true" class="jws-icon-caret-right-thin"></i></span>',
          nextArrow:
            '<span class="jws-carousel-btn next-item "><i aria-hidden="true" class="jws-icon-caret-right-thin"></i></span>',
        });

        $(window).resize(function () {
          jwsThemeModule.fixHeight($(".related-slider .product-item-inner"));
        });
        jwsThemeModule.fixHeight($(".related-slider .product-item-inner"));
      },
      ...
      jws_product_tabs_block: function () {
        if ($(".woocommerce-tabs").hasClass("tabs_modal")) {
          $(".jws-group-accordion-wap")
            .appendTo("body")
            .wrap('<div class="tabs-modal-container"></div>');
          $(".tabs-modal-container").append(
            '<span class="overlay"></span><span class="close jws-icon-cross"></span>'
          );
          ...
          $(document).on(
            "click",
            ".tabs-modal-container .overlay , .tabs-modal-container .close",
            function () {
        ...
        $(document).on("click", ".tabs li a", function () {
          $(".jws-group-accordion.accordion-active").removeClass(
            "accordion-active"
          );
          $($(this).attr("href"))
            .closest(".jws-group-accordion")
        ...
        $(document).on(
          ...
          function () {
            ...
            $(".woocommerce-tabs .accordion-active").removeClass(
              "accordion-active"
            );
            if (!tab_is_activated) {
              $(this)
              ...
              if ($(".woocommerce-tabs .tabs.wc-tabs")[0]) {
                ...
                $(".woocommerce-tabs .tabs.wc-tabs .active").removeClass(
                  "active"
                );
                $('.woocommerce-tabs .tabs.wc-tabs a[href="#' + id + '"]')
                  .parent()
                  .addClass("active");
              }
            }
        ...
        $(".jws_action_review span").on("click", function (e) {
          $(".jws_action_review span , .tabs_review_questios span").removeClass(
            "active"
          );
          $("html,body").animate(
            {
            ...
            .success(function (response) {
              if (response.status == "error") {
                $(".form-questions").append(
                  '<div class="error">' + response.note + "</div>"
                );
              } else {
        ...
        if ($("#answer-question-list").attr("data_question_url")) {
          var load_pagination =
            $("#answer-question-list").attr("data_question_url") +
            "?product_questions=" +
            $("#answer-question-list").attr("data_product_id") +
            "";
          ...
          $.get(
            load_pagination,
            function (res) {
              $("#answer-question-list .jws-pagination-number").html(
                $(res).find(".jws-pagination-number").html()
              );
            },
            "html"
          );
        ...
        $(document.body).on(
          "click",
          "#answer-question-list .jws-pagination-number a",
          function (e) {
            e.preventDefault();
            if (
              !$(this).parents("#answer-question-list").find(".loader").length
            ) {
              $(this)
                .parents("#answer-question-list")
                .append(
                  '<div class="loader"><svg class="circular" viewBox="25 25 50 50"><circle class="path" cx="50" cy="50" r="20" fill="none" stroke-width="2" stroke-miterlimit="10"/></svg></div>'
                );
            }
        ...
        $(document.body).on(
          "click",
          "#comments .jws-pagination-number a",
          function (e) {
            e.preventDefault();
            ...
            $.get(
              url,
              function (res) {
                $("#comments .jws-pagination-number").html(
                  $(res).find("#comments .jws-pagination-number").html()
                );
                $("#comments > .commentlist").html(
                  $(res).find("#comments > .commentlist").html()
                );
      ...
      woocommerceCategoryWidget: function () {
        $(".product-categories .cat-parent ").append(
          "<span class='cat_btn lnr lnr-chevron-down'></span>"
        );
        $("body").on(
          "click",
          ".product-categories .cat-parent > .cat_btn",
          function (e) {
            var t = $(this).parent(),
              n = $(t).parent();
            if ($(t).hasClass("active"))
              $(t).find("> ul").slideUp(200).parent().removeClass("active");
            else {
              ...
              $(s)
                ...
                .slideUp(200),
                $(t).children("ul").slideDown(200).parent().addClass("active");
            }
            return !1;
          }
        );
      ...
      swatchesVariations: function () {
          ...
          $variation_form
            ...
            .on("show_variation", function (e, variation, purchasable) {
              ...
              text_vari
                ...
                .prev(".label")
                .append(
                  '<span class="text_vari">' +
                    $variation_form
                      .find(".swatch-color.selected")
                      .data("value") +
                    "</span>"
                );
              ...
              if (!isVariationGallery(variation.variation_id)) {
                ...
                if (
                  !isQuickShop($variation_form) &&
                  !replaceMainGallery(variation.variation_id, $variation_form)
                ) {
                  if ($firstThumb.attr("src") != variation.image.thumb_src) {
                    $firstThumb.wc_set_variation_attr(
                      "src",
                      variation.image.src
                    );
                  }
        ...
        var scrollToTop = function () {
          if (
            !$("body").hasClass("meathouseduct-design-sticky") ||
      ...
      productImagesGallery: function () {
        ...
        if ($slider_wap.hasClass("layout_gid_two_coloms")) {
          return false;
        }
        if (
          $slider_wap.hasClass("resize-slider-mobile") &&
          $(window).width() < 767
        ) {
          initMainGallery();
        }
          ...
          $mainGallery
            ...
            .each(function () {
              ...
              if ($(this).hasClass("video_inner")) {
                markup +=
                  '<div class="product-image-thumbnail"><div class="video_thumbnail"><iframe class="slide-media" src="' +
                  $(".slide-media").attr("src") +
                  '"></iframe></div></div>';
              } else {
                markup +=
          ...
          if ($(".has_video").hasClass("inner")) {
            setTimeout(function () {
              $thumbs
                .find(".slide-media")
                .get(0)
                .contentWindow.postMessage(JSON.stringify(comand), "*");
            }, 2000);
          }
        }
      ...
      quickViewInit: function () {
        ...
        $(document).on("click", ".quickview-button", function (e) {
          ...
          jwsThemeWooModule.quickViewLoad(
            productId,
            btn,
            prev,
            next,
      ...
      share_button: function () {
        ...
        $(".product-share button").on("click", function () {
          ...
          if (
            parents.find(".addthis_inline_share_toolbox").hasClass("opened")
          ) {
            parents
      ...
      filterAjax: function () {
        ...
        $(window).on("resize scroll", function () {
          if (
            $(".auto_load_more").length &&
            $(".auto_load_more").isInViewport() &&
            autoload
          ) {
            autoload = false;
              ...
              function (res) {
                ...
                intervalID = setInterval(function () {
                  $(".products-wrap")
                    .find(".product-item")
                    .eq(iter)
                    .addClass("jws-animated");
                  iter++;
                }, 100);

                autoload = true;

                $("#jws-shop-topbar").html(
                  $(res).find("#jws-shop-topbar").html()
                );
                $(".siderbar-inner").html(
                  $(res).find(".siderbar-inner").html()
                );
                $(".woocommerce-pagination").html(
                  $(res).find(".woocommerce-pagination").html()
                );
                $(".filter-shop-nav").html(
                  $(res).find(".filter-shop-nav").html()
                );
                $(".shop-filter-actived").html(
                  $(res).find(".shop-filter-actived").html()
                );
                $(".woocommerce-result-count").html(
                  $(res).find(".woocommerce-result-count").html()
                );
        ...
        $(document.body).on(
          "click",
          "form.woocommerce-ordering div.orderby ul span",
          function (e) {
            e.preventDefault();
            ...
            $("form.woocommerce-ordering div.orderby ul span").removeClass(
              "current"
            );
        ...
        $(document.body).on(
          ...
          function (e) {
            ...
            $(document.body).trigger("meathouse_catelog_filter_ajax", [
              url,
              $(this),
            ]);
          }
        ...
        $(document.body)
          .find("#jws-shop-product-cats")
          .on("click", ".cat-link", function (e) {
            e.preventDefault();
            var url = $(this).attr("href");
            $(document.body).trigger("meathouse_catelog_filter_ajax", [
              url,
        ...
        $(document.body)
          .find("#jws-shop-topbar , .siderbar-inner , .filter-shop-nav")
          .on("click", "a", function (e) {
        ...
        $(document.body).on(
          ...
          function (e, url, element) {
            ...
            $(document.body).trigger(
              "meathouse_ajax_filter_before_send_request"
            );
            clearInterval(intervalID);
            $.get(
              url,
              function (res) {
                ...
                $(".jws-breadcrumbs").html(
                  $(res).find(".jws-breadcrumbs").html()
                );
                $(".shop-filter-actived").html(
                  $(res).find(".shop-filter-actived").html()
                );
                ...
                $(document.body).trigger(
                  "meathouse_ajax_filter_request_success",
                  [res, url]
                );
              },
              "html"
        ...
        $(document.body).on(
          ...
          function () {
            if (
              $("#jws-shop-toolbar").hasClass("on-mobile") ||
              $("#jws-shop-topbar").hasClass("on-mobile")
            ) {
  ...
  jQuery(document).ready(function () {
    $(".siderbar-inner .widget .widget-title").click(function (e) {
      e.preventDefault();
